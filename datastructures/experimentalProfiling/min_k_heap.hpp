...
template <typename T>
void MinKHeap<T>::insert_bottomup(T val){
...
//Return: Returns void
template <typename T>
void MinKHeap<T>::insert_upheap(T val){
	m_array[num_elements] = val;
	num_elements++;
			...
			do{
				...
				for(int j=1; j<=5;j++){
					if(num_elements > (5*check_value+j)){
						swap_value = m_array[check_value] - m_array[5*check_value+j];
...
template <typename T>
int MinKHeap<T>::findMin(){
	if(!isEmpty()){
		//std::cout << "Min Value: "<<m_array[0]<<"\n";
		return 0;
	}else{
		return -1;
	}
	...
	if(!isEmpty()){
		if(num_elements > 1){
			...
			for(int i=((num_elements-2)/k); i < num_elements; i++){
				if(m_array[i] > greatest_val){
					greatest_val = m_array[i];
					greatest_index = i;
				}
			}
...
//Return: true if deleted, false otherwise
template <typename T>
bool MinKHeap<T>::deleteMax(){
	if(!isEmpty()){
		if(num_elements > 1){
			//Find min location then shift array because max value will be a leaf
			int max_location = findMax();
			m_array[max_location] = m_array[num_elements-1];
			bool continue_check = true;
			int check_index = max_location;
